      ...
      <div className="bg-gradient-to-r from-blue-500 via-red-500 to-yellow-500 py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            <h1 className="text-4xl md:text-5xl font-bold text-white mb-4">{t('home.features.radios.title')}</h1>
            <p className="text-xl text-white/90 max-w-2xl mx-auto">{t('home.features.radios.desc')}</p>
          </div>
        </div>
      </div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {stations.map((s) => (
            <div key={s.id} className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-6 shadow-sm">
              ...
              <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">{t('radios.quality', 'MP3 quality when available.')}</p>
              <button
                onClick={() => play(s)}
                ...
                aria-label={`${t('radios.play', 'Play')} ${s.name}`}
              >
                <Play className="w-4 h-4" />
                {currentStation?.id === s.id && isPlaying ? t('radios.playing', 'Playing') : t('radios.play', 'Play')}
              </button>
            </div>
          ))}
        </div>
